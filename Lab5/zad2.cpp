...
int main()
{
    ...
    if (fork() == 0)
    { // utworzenie procesu potomnego ktory bedzie wyswietlal
        execlp("display", "display", "-update", "1", "plik_zapis", NULL); // polecenie wyswietlenia aktualnego obrazka bedacego w plik_zapis
    }
    ...
    while (true) // nieskonczona petla
    {
        ...
        msync(plik_pamiec, stan_pliku.st_size, MS_SYNC); // aktualizacja wyswietlanego obrazu przez podproces
                                                         // flaga MS_SYNC zada aktualizacji
